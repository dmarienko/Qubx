...
from qubx.utils.plotting.data import PlotData, PlotStyle, SeriesData, SubplotData
...
def test_series_data_initialization(sample_line_data):
    series = SeriesData(data=sample_line_data, type="line", name="test_series", style=PlotStyle(color="blue"))
...
def test_plot_data_line_conversion(sample_line_data):
    series = SeriesData(data=sample_line_data, type="line", name="MA", style=PlotStyle(color="blue", width=2.0))
    subplot = SubplotData(series=[series])
    plot = PlotData(main=subplot)
...
def test_plot_data_with_studies(sample_ohlcv_data, sample_line_data):
    ...
    plot = PlotData(main=main_subplot, studies={"MA Study": study_subplot})
