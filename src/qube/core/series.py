...
class Float(TimeItem):
    ...
    def __repr__(self):
        return f"[{time_to_str(self.time, 's')}]\t{self.value} { f'({str(self.info)})' if self.info else '' }"
...
class TimeSeries:
    ...
    def _update_indicators(self, item: TimeItem, is_new_item: bool):
        pass
    ...
    def __str__(self):
        nl = len(self)
        r = f"TimeSeries[{self.name} / {str(self._timeframe)}] | {nl} records\n"
        hd, tl = 3, 3 
        if nl < hd + tl:
            hd, tl = nl, 0
...
class FloatSeries(TimeSeries):
    ...
    def _convert(self, time: dt64_t, item: TimeItem) -> TimeItem:
        if isinstance(item, Float):
            ...
            raise ValueError(f"Can't process value {item} !")
        return Float(time, v)
